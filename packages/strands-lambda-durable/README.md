...
- **What is replayed.** Completed model calls and tool uses are replayed from the journal, not executed again. This includes error results, interrupts, `appState` changes, and `modelState`. Binary content (`Uint8Array`) is kept. Checkpoints are versioned: `schemaVersion` 2, and version 1 is still read.
