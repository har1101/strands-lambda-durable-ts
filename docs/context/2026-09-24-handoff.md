...
| [har1101/strands-lambda-durable-ts](https://github.com/har1101/strands-lambda-durable-ts)（このリポジトリ） | デプロイできるチャットアプリのサンプルと、調査メモ | `main` にマージ済み（PR #1〜#3） |
...
- **2026-09-24 に、`main` の最新コードを再デプロイしました。** 反映された内容は次のとおりです。
  ...
  - 確定済みメッセージだけを返す API
  - 承認待ちのポーリングのバックオフ
  - ライブラリを v0.1.1 の tarball から読み込む構成
...
- **npm 公開**: `NPM_TOKEN` を設定するかどうか（2 章）。設定しない場合、利用者は GitHub Release の tarball の URL からインストールします。
...
- **決めること**:
  ...
  - 名前（`strands-` 接頭辞は外す）
  - 現在の `strands-lambda-durable` をコアの上に作り直すかどうか
